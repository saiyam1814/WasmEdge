...
TEST(APICoreTest, Compiler) {
  ...
  EXPECT_TRUE(WasmEdge_ResultOK(WasmEdge_CompilerCompileFromBytes(
      Compiler,
      WasmEdge_BytesWrap(Data.data(), static_cast<uint32_t>(Data.size())),
      "test_aot" WASMEDGE_LIB_EXTENSION)));
  // Check the header of the output files.
  OutFile.open("test_aot" WASMEDGE_LIB_EXTENSION, std::ios::binary);
  EXPECT_TRUE(OutFile.read(reinterpret_cast<char *>(Buf), 4));
  OutFile.close();
